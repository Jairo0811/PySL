...
class GuessView(QWidget):
    def __init__(self, db: Database) -> None:
        ...
        layout.addWidget(QLabel("Adivina un número entre 1 y 100"))
        self.value = QSpinBox()
        self.value.setRange(1, 100)
        layout.addWidget(self.value)
        self.result = QLabel()
        button = QPushButton("Probar")
        button.clicked.connect(self.guess)
        layout.addWidget(button)
        layout.addWidget(self.result)
        layout.addStretch()

    def guess(self):
        result = self.game.guess(self.value.value())
        self.result.setText(
            {
                "mayor": "El número secreto es mayor.",
                "menor": "El número secreto es menor.",
                "correcto": f"¡Correcto en {self.game.attempts} intentos!",
            }[result]
        )
        if result == "correcto":
            self.db.record_game("guess", True)
            self.game = GuessNumberGame()
...
class RpsView(QWidget):
    def __init__(self, db: Database) -> None:
        ...
        self.choice.addItems(["piedra", "papel", "tijera"])
        self.result = QLabel()
    ...
    def run(self):
        result, computer = play(self.choice.currentText())
        self.result.setText(f"Computadora: {computer}. Resultado: {result}.")
        self.db.record_game("rps", result == "ganaste")
...
class TicView(QWidget):
    ...
    def move(self, index):
        try:
            winner = self.game.move(index)
            self.buttons[index].setText(self.game.board[index])
        except ValueError:
            return
        if winner:
            self.status.setText(f"Resultado: {winner}")
            self.db.record_game("tic_tac_toe", winner in {"X", "O"})
            for b in self.buttons:
                b.setEnabled(False)
        else:
            self.status.setText(f"Turno: {self.game.turn}")

    def reset(self):
        self.game = TicTacToe()
        self.status.setText("Turno: X")
        for b in self.buttons:
            b.setText(" ")
            b.setEnabled(True)
